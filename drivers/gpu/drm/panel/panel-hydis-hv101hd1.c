...
static int hv101hd1_on(struct hv101hd1 *hv)
{
...
static int hv101hd1_off(struct hv101hd1 *hv)
{
	struct mipi_dsi_device *dsi = hv->dsi;
	struct device *dev = &dsi->dev;
	int ret;
...
static int hv101hd1_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_enable(hv->supply);
	if (ret < 0) {
...
static int hv101hd1_probe(struct mipi_dsi_device *dsi)
{
	...
	hv->supply = devm_regulator_get(dev, "power");
	if (IS_ERR(hv->supply))
		return dev_err_probe(dev, PTR_ERR(hv->supply),
	...
	/* panel has CONTINUOUS_MODE and NONE_BURST_MODE */
...
static int hv101hd1_remove(struct mipi_dsi_device *dsi)
{

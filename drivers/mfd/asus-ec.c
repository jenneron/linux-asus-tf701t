...
static int asusec_signal_request(const struct asusec_info *ec)
{
	...
	/* if request gpio is not set, skip this part */
	if (!priv->ecreq)
		goto unlock;
...
static int asus_ec_probe(struct i2c_client *client)
{
	...
	/*
	 * The asus-ec may not have an interrupt. So in the case it wasn't provided we
	 * use polling in order get the controller's status.
	 */
	if (priv->self->irq) {
